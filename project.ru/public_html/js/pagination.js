document.addEventListener("DOMContentLoaded", function () {
  ...
  // 🔹 Обновляем визуальное состояние кнопок (без замены DOM)
  ...
  function loadPage(page) {
    ...
    fetch(newUrl)
      ...
      .then((html) => {
        ...
        const newPosts = doc.querySelector(".slider-post");
  ...
  // 🔹 Обработчики для карточек постов (если нужны)
  function initPostHandlers() {
    document.querySelectorAll(".post-card").forEach((card) => {
      // Здесь можно добавить клики по постам, лайки и т.д.
    });
  }
  ...
  if (sliderPost) {
    sliderPost.style.transition = "opacity 0.15s ease";
  }
});
